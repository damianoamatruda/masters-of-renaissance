...
public class FaithTrack extends StringComponent {
    ...
    public String getString() {
        ...
        int cellWidth = 5; // Integer.max(6, points.keySet().stream().map(String::length).reduce(Integer::max).orElse(0));
        ...
        for (int j = 0; j < players.size() + (players.size() == 1 ? 1 : 0); j++) {
            ...
            if (yellowTiles.contains(maxFaith))
                stringBuilder.append("\u001B[93m");
            else if (sectionTiles.contains(maxFaith))
