...
2. Incorrect messages (messages that when executed by the server return an exception, e.g. due to an incorrect choice)
3. Server not responding
4. Player disconnecting
...
Incorrect messages generate an exception, the message of which is sent back to the client the original message originated from.  
`ErrAction` messages have the following structure:
```json
{
  "type": "ErrAction",
  "msg": "<error message>"
...
```
Example message contents are written in each section.
...
Uncaught exceptions are signaled to the client the message which caused the exception came from by an `ErrController` message:
```json
{
  "type": "ErrController",
  "msg": "<error message>"
